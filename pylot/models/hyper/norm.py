...
class HyperWeightNorm(nn.Module):
    def __init__(self, weight_shape):
        super().__init__()
        self.g = nn.Parameter(torch.ones(weight_shape))
...
class NormHyperNet(nn.Module):
    ...
    def __post_init__(self, reset=True):
        ...
        for i, (in_size, out_size) in enumerate(zip(sizes, sizes[1:])):
            ...
            if self.layer_norm:
                self.layers.add_module(f"l{i}_ln", nn.LayerNorm(out_size))

        self.outputs = nn.ModuleDict()
        if self.output_weight_norm:
            self.outputs_norm = nn.ModuleDict()
        for k, size in self.output_sizes.items():
            ...
            if self.weight_norm:
                lin = nn.utils.weight_norm(lin)
            if self.output_weight_norm:
                self.outputs_norm[k.replace(".", ":")] = HyperWeightNorm(flat_size)
    ...
    def forward(self, **inputs: Dict[str, Tensor]):
        ...
        if self.output_weight_norm:
            output = {
                k: self.outputs_norm[k.replace(".", ":")](
                    self.outputs[k.replace(".", ":")](intermediate)
                ).view(self.output_sizes[k])
